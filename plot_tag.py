...
def plot_all(exp_data):
    ...
    plt.xticks([0, 200_000, 400_000, 600_000, 800_000, 900_000], ["0.0", "0.2", "0.4", "0.6", "0.8", "0.9"])
    plt.xlabel("Timesteps (in millions)", fontsize=26, fontweight="bold")
    plt.ylabel("Test return mean", fontsize=26, fontweight="bold")
    plt.tick_params(axis='x', labelsize=20)
    plt.tick_params(axis='y', labelsize=20)
    plt.title("Tag", fontsize=36, fontweight="bold", y=1.02)
    plt.legend(loc="upper left", fontsize=20)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig("tag_results_plot.png", dpi=300)
    plt.show()
